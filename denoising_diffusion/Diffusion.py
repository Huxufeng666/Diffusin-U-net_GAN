...
class DiffusionModule(nn.Module):
    """
    简单实现：利用固定高斯模糊算子对输入图像进行背景模糊，
    输出 = image * mask + blurred(image) * (1 - mask)
    """
    def __init__(self, kernel_size=5, sigma=1.0):
        ...
        self.sigma = sigma
        # 定义高斯模糊卷积
        self.blur_conv = nn.Conv2d(1, 1, kernel_size, padding=kernel_size//2, bias=False)
        gaussian_kernel = self.get_gaussian_kernel(kernel_size, sigma)
        self.blur_conv.weight.data.copy_(gaussian_kernel)
        # 固定卷积层参数（不训练）
    ...
    def forward(self, image, mask):
        blurred = self.blur_conv(image)
        # mask 的取值范围假设为 [0,1]（1表示前景：肿瘤区域）
        output = image * mask + blurred * (1 - mask)
        return output



class SimpleGaussianBlur(nn.Module):
    """
    与 DiffusionModule 功能相同：固定高斯卷积 + 前景保持 + 背景模糊
    """
    def __init__(self, kernel_size=5, sigma=1.0):
        super(SimpleGaussianBlur, self).__init__()
        self.kernel_size = kernel_size
        self.sigma = sigma
        ...
        with torch.no_grad():
            self.blur_conv.weight.copy_(gaussian_kernel)
    ...
    def forward(self, image, mask):
        """
        image: shape [B,1,H,W] (单通道为例)
        mask: shape [B,1,H,W] in [0,1]
        返回: 前景保持、背景模糊后的图像
        """
        blurred = self.blur_conv(image)
        output = image * mask + blurred * (1 - mask)
...
class ContinuousTimeGaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.blur = SimpleGaussianBlur(kernel_size, sigma)
    ...
    def p_losses(self, x_start, times, mask=None, **kwargs):
        noise = default(noise, lambda: torch.randn_like(x_start))
