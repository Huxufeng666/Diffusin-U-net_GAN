...
# 假设已经定义了 MedicalDataset, EndToEndModel, Discriminator, CombinedLoss, HausdorffDTLoss, parse_args 等
...
#     # 定义一个列表保存最佳权重 (测试损失, 权重文件路径)
#     best_models = []  # 最多存储3个最好的权重
...
#         # 保存当前 epoch 模型权重
#         current_weight_file = os.path.join(weights_dir, f"model_epoch_{epoch+1}.pth")
#         torch.save(model.state_dict(), current_weight_file)

#         # 如果有测试损失，则更新最佳模型列表（目标：保留测试损失最低的3个权重）
#         if avg_test_loss is not None:
#             # 将 (avg_test_loss, 文件路径) 添加到列表中
#             best_models.append((avg_test_loss, current_weight_file))
#             # 按照测试损失升序排序（最好的在前面）
#             best_models.sort(key=lambda x: x[0])
#             # 如果列表长度超过3，删除测试损失最高的那个权重
#             if len(best_models) > 3:
#                 # 弹出最后一个，即最差的权重文件
#                 worst_loss, worst_file = best_models.pop()
#                 if os.path.exists(worst_file):
#                     os.remove(worst_file)
#                     print(f"Deleted worst model: {worst_file} with loss {worst_loss:.4f}")
...
#     # 输出最终保留的最佳模型
#     print("Best models:")
#     for loss_val, file_path in best_models:
#         print(f"Loss: {loss_val:.4f}, Path: {file_path}")
